(function(){
  ...
  function accidentChance(player){const base=state.chaos<30?.30:state.chaos<60?.42:state.chaos<80?.55:.68;let chance=Math.min(.82,Math.max(.18,base+(player===BLACK?.07:-.05)));if(player===BLACK&&state.luckyGuard)chance=Math.max(.08,chance-.30);return chance;}
