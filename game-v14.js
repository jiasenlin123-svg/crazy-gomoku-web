(function(){
  ...
  const MODES={
    easy:{
      id:'easy',icon:'🌱',label:'简单',description:'意外较少 · 4 次化解 · 道具更充足',
      startChaos:8,rescues:4,playerRates:[.16,.22,.30,.38],aiRates:[.13,.18,.24,.30],
      badWeight:1.6,goodWeight:1.25,chaoticWeight:1.1,special:1,
      moveChaos:3,eventChaos:6,rescueCost:5,aiAttack:.92,aiDefense:.98,aiPick:5,
      items:{spray:2,ice:1,lucky:1}
    },
    normal:{
      id:'normal',icon:'🎯',label:'正常',description:'随机与策略平衡 · 推荐默认游玩',
      startChaos:12,rescues:3,playerRates:[.22,.30,.40,.50],aiRates:[.17,.23,.30,.38],
      badWeight:2.6,goodWeight:.85,chaoticWeight:1.4,special:1.15,
      moveChaos:4,eventChaos:8,rescueCost:8,aiAttack:1.02,aiDefense:1.06,aiPick:2,
      items:{spray:1,ice:1,lucky:1}
    },
    hard:{
      id:'hard',icon:'☠️',label:'困难',description:'负面事件权重更高 · 挑战赢一把',
      startChaos:18,rescues:3,playerRates:[.28,.36,.46,.56],aiRates:[.20,.28,.36,.44],
      badWeight:4.2,goodWeight:.45,chaoticWeight:1.8,special:1.35,
      moveChaos:5,eventChaos:10,rescueCost:12,aiAttack:1.12,aiDefense:1.15,aiPick:1,
      items:{spray:1,ice:1,lucky:1}
    }
  };
  ...
  const state={
    ...
    eventLog:[],chaos:12,moves:0,soundEnabled:true,decisionUnlockTimer:null,
    forceOpponentRush:null,attempts:1,wins:0,losses:0,mode:'normal',
  ...
  const modeEasy=$('modeEasy'),modeNormal=$('modeNormal'),modeHard=$('modeHard'),modeBadge=$('modeBadge'),modeDescription=$('modeDescription'),accidentBottomTape=$('accidentBottomTape');

  let audioContext=null;
  loadStats();

  function cfg(){return MODES[state.mode]||MODES.normal;}
  function emptyBoard(){return Array.from({length:SIZE},()=>Array(SIZE).fill(EMPTY));}
  function loadStats(){try{const x=JSON.parse(localStorage.getItem('crazyGomokuChallenge')||'{}');state.attempts=Math.max(1,x.attempts||1);state.wins=x.wins||0;state.losses=x.losses||0;if(MODES[x.mode])state.mode=x.mode;}catch(_){}}
  function saveStats(){try{localStorage.setItem('crazyGomokuChallenge',JSON.stringify({attempts:state.attempts,wins:state.wins,losses:state.losses,mode:state.mode}));}catch(_){}}
  ...
  function updateChallengeUI(){const m=cfg();if(challengeAttempt)challengeAttempt.textContent=`第 ${state.attempts} 次挑战`;if(challengeRecord)challengeRecord.textContent=`${state.wins} 胜 / ${state.losses} 负`;if(modeBadge){modeBadge.textContent=`${m.icon} ${m.label}`;modeBadge.className=`mode-badge mode-${state.mode}`;}if(modeDescription)modeDescription.textContent=m.description;if(accidentBottomTape)accidentBottomTape.textContent=`⚠ 化解一次 · 疯狂值额外 +${m.rescueCost}% ⚠`;document.title=`疯狂五子棋｜${m.label}模式｜挑战赢一把`;document.body.dataset.mode=state.mode;[[modeEasy,'easy'],[modeNormal,'normal'],[modeHard,'hard']].forEach(([btn,id])=>{if(!btn)return;btn.classList.toggle('active',state.mode===id);btn.setAttribute('aria-pressed',state.mode===id?'true':'false');btn.disabled=state.overlayActive||!!state.pending||state.aiThinking;});}
  function stageIndex(){return state.chaos<30?0:state.chaos<60?1:state.chaos<80?2:3;}
  function stageInfo(){const m=cfg(),i=stageIndex(),names=['开局','开始失控','高危','疯狂时刻'],notes=['还有较多正常落子空间','随机性明显增强','重度 Debuff 出现','高危但仍有操作空间'];return[`${m.label}${names[i]}`,`玩家意外约 ${Math.round(m.playerRates[i]*100)}% · ${notes[i]}`];}
  ...
  function accidentChance(player){const m=cfg(),i=stageIndex();let chance=player===BLACK?m.playerRates[i]:m.aiRates[i];if(player===BLACK&&state.luckyGuard)chance=Math.max(.06,chance-.30);return chance;}
  ...
  function eventWeight(ev,player){const m=cfg(),tierBoost=state.chaos>=80?(ev.tier===HEAVY?1.8:1):state.chaos>=60?(ev.tier===HEAVY?1.35:1):1;let biasWeight=1;if(player===BLACK){biasWeight=ev.bias===BAD?m.badWeight:ev.bias===GOOD?m.goodWeight:m.chaoticWeight;}else{biasWeight=ev.bias===BAD?1.35:ev.bias===GOOD?2.2:1.8;}if(player===BLACK&&(ev.id==='wild'||ev.id==='flip'||ev.id==='opponentRush'))biasWeight*=m.special;return tierBoost*biasWeight;}
  ...
  function selectMode(id){if(!MODES[id]||id===state.mode||state.overlayActive||state.pending||state.aiThinking)return;state.mode=id;saveStats();resetGame(false);const m=cfg();showSideEvent({icon:m.icon,title:`已切换：${m.label}模式`,lead:m.description},false);render();}
  if(modeEasy)modeEasy.addEventListener('click',()=>selectMode('easy'));
  if(modeNormal)modeNormal.addEventListener('click',()=>selectMode('normal'));
  if(modeHard)modeHard.addEventListener('click',()=>selectMode('hard'));
  ...
  function showOverlay(pending,canRescue){const{event:ev,player,row,col}=pending,m=cfg();state.overlayActive=true;accidentIcon.textContent=ev.icon;accidentTitle.textContent=`${ev.title}！`;accidentLead.textContent=ev.lead;accidentNext.textContent=ev.next;accidentTarget.textContent=`🎯 ${player===BLACK?'你原本想下':'AI 原本想下'}：${toCoord(row,col)}`;
    ...
    if(canRescue){overlayAccept.disabled=true;overlayRescue.disabled=true;overlayRescue.textContent=`🛠 立即化解 ×${state.playerRescues}`;setFoot('📢 意外播报中……');clearTimeout(state.decisionUnlockTimer);state.decisionUnlockTimer=setTimeout(()=>{overlayAccept.disabled=false;overlayRescue.disabled=state.playerRescues<=0;setFoot(`⏸ 你来决定什么时候继续 · 化解会额外 +${m.rescueCost} 疯狂值`);},DECISION_LOCK_MS);}else{accidentAuto.disabled=true;accidentAuto.textContent='📢 AI 触发意外 · 正在播报……';setFoot('⏸ AI 不会自动继续');clearTimeout(state.decisionUnlockTimer);state.decisionUnlockTimer=setTimeout(()=>{accidentAuto.disabled=false;accidentAuto.textContent='🎬 我看懂了，让 AI 继续';setFoot('⏸ 你不点，棋局就停在这里');},DECISION_LOCK_MS);}
    document.body.classList.add('chaos-shake');setTimeout(()=>document.body.classList.remove('chaos-shake'),500);sound('accident');updateChallengeUI();
  }
  function hideOverlay(){state.overlayActive=false;accidentOverlay.classList.add('hidden');updateChallengeUI();}
  ...
  function rescueCurrent(){if(!state.pending||state.playerRescues<=0)return;const p=state.pending,cost=cfg().rescueCost;state.pending=null;state.playerRescues--;bumpChaos(cost);hideOverlay();sound('rescue');showSideEvent({icon:'🛠️',title:'化解成功',lead:`救回这一步，疯狂值额外上涨 ${cost}%。`},false);setTimeout(()=>placeStone(BLACK,p.row,p.col),560);render();}
  ...
  function chooseAiMove(){const m=cfg(),cand=[];for(let r=0;r<SIZE;r++)for(let c=0;c<SIZE;c++){if(state.board[r][c]!==EMPTY)continue;let score=evaluate(r,c,WHITE)*m.aiAttack+evaluate(r,c,BLACK)*m.aiDefense+(7-(Math.abs(7-r)+Math.abs(7-c))*.08)+Math.random()*.3;cand.push({r,c,score});}cand.sort((a,b)=>b.score-a.score);if(!cand.length)return null;const top=Math.min(m.aiPick,cand.length),pick=cand[Math.floor(Math.random()*top)];return[pick.r,pick.c];}
  ...
  function finish(player){if(state.gameOver)return;state.gameOver=true;hideOverlay();if(player===BLACK){state.wins++;sound('win');winEmoji.textContent='🏆';winTitle.textContent='挑战成功！';winText.textContent=`${cfg().label}模式 · 第 ${state.attempts} 次挑战，赢下一把！`;}else{state.losses++;sound('lose');winEmoji.textContent='🤖';winTitle.textContent='挑战失败';winText.textContent=`${cfg().label}模式没顶住。再来一把！`;}saveStats();render();winOverlay.classList.remove('hidden');}
  ...
  function resetGame(countAttempt=true){clearTimeout(state.decisionUnlockTimer);if(countAttempt&&state.moves>0){state.attempts++;saveStats();}const m=cfg();state.board=emptyBoard();state.current=BLACK;state.gameOver=false;state.playerRescues=m.rescues;state.pending=null;state.overlayActive=false;state.aiThinking=false;state.extraTurn=false;state.forceOpponentRush=null;state.lastMove=null;state.eventLog=[];state.chaos=m.startChaos;state.moves=0;state.items={spray:m.items.spray,ice:m.items.ice,lucky:m.items.lucky};state.luckyGuard=false;state.supply60=false;state.supply85=false;winOverlay.classList.add('hidden');accidentOverlay.classList.add('hidden');showSideEvent({icon:m.icon,title:`${m.label}模式开始`,lead:m.description},false);renderLog();render();}
  ...
  window.CrazyGomoku={checkWin,createEmptyBoard:emptyBoard,SIZE,BLACK,WHITE,EMPTY,MODES};
