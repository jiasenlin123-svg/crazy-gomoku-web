(function(){
  ...
  function render(){const cells=boardEl.children;for(let i=0;i<cells.length;i++){const r=Math.floor(i/SIZE),c=i%SIZE,cell=cells[i];const old=cell.querySelector('.stone');if(old)old.remove();const v=state.board[r][c];if(v!==EMPTY){const s=document.createElement('span');s.className=`stone ${v===BLACK?'black':'white'}`;if(state.lastMove&&state.lastMove[0]===r&&state.lastMove[1]===c)s.classList.add('new');cell.appendChild(s);}}
    ...
    updateChallengeUI();updateChaosUI();
  }
  ...
  function stageInfo(){if(state.chaos<30)return['困难开局','玩家意外约 37% · 负面事件权重更高'];if(state.chaos<60)return['开始失控','玩家意外约 49% · 落子随机范围扩大'];if(state.chaos<80)return['高危','玩家意外约 62% · 重度 Debuff 出现'];return['疯狂时刻','玩家意外约 75% · 系统开始疯狂针对'];}
  ...
  function bumpChaos(n){state.chaos=Math.min(100,state.chaos+n);updateChaosUI();}
  function accidentChance(player){const base=state.chaos<30?.30:state.chaos<60?.42:state.chaos<80?.55:.68;return Math.min(.82,Math.max(.18,base+(player===BLACK?.07:-.05)));}
  ...
  function attemptMove(player,r,c,canRescue){const ev=maybeEvent(player);if(!ev){placeStone(player,r,c);return;}bumpChaos(10);const pending={event:ev,player,row:r,col:c};state.pending=pending;logEvent(ev,player);showSideEvent(ev,true);showOverlay(pending,canRescue&&player===BLACK);}
